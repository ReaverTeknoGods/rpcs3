...
{
	// usio_log.notice("Translate Input 0x1080");
...
void usb_device_usio::usio_write(u8 channel, u16 reg, std::vector<u8>& data)
{
	...
	if (channel == 0)
	{
		//usio_log.notice("Usio write to channel 0(reg: 0x%04X, size: 0x%04X, data: %s)", reg, data.size(), fmt::buf_to_hexstring(data.data(), data.size()));
		switch (reg)
		{
		case 0x0002:
		{
		...
		default:
		{
			usio_log.notice("Unhandled channel 0 register write(reg: 0x%04X, size: 0x%04X, data: %s)", reg, data.size(), fmt::buf_to_hexstring(data.data(), data.size()));
			break;
		}
		}
	}
...
{
	//usio_log.notice("Interrupt transfer on endpoint 0x%02X, buf_size: %d", endpoint, buf_size);
