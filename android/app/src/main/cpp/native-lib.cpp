...
#include "Input/android_pad_handler.h"
...
extern "C" JNIEXPORT void JNICALL Java_net_rpcs3_RPCS3_arcadeInput(
  ...
  const std::array<u8, 7> analog = {
      ...
      static_cast<u8>(std::clamp(analog2, 0, 255)),
      static_cast<u8>(std::clamp(analog3, 0, 255)),
...
extern "C" JNIEXPORT jboolean JNICALL Java_net_rpcs3_RPCS3_configureArcadeRoot(
    JNIEnv *env, jobject, jstring root_path, jstring vfs_config_path) {
  if (Emu.GetStatus(false) != system_state::stopped || root_path == nullptr ||
      vfs_config_path == nullptr) {
    return false;
  }
  const std::string root = unwrap(env, root_path);
  const std::string config_path = unwrap(env, vfs_config_path);
  if (root.empty() || config_path.empty()) {
    return false;
  }

  fs::file config_file{config_path, fs::read};
  if (!config_file) {
    rpcs3_android.error("TeknoParrot VFS config is missing: %s", config_path);
    return false;
  }

  const std::string shared_dev_flash = g_cfg_vfs.get_dev_flash();
  const std::string shared_dev_flash2 = g_cfg_vfs.get_dev_flash2();
  const std::string shared_dev_flash3 = g_cfg_vfs.get_dev_flash3();
  if (!g_cfg_vfs.from_string(config_file.to_string())) {
    rpcs3_android.error("TeknoParrot VFS config is invalid: %s", config_path);
    return false;
  }

  g_cfg_vfs.emulator_dir.from_string("");
  g_cfg_vfs.dev_hdd0.from_string(root + "/dev_hdd0/");
  g_cfg_vfs.dev_hdd1.from_string(root + "/dev_hdd1/");
  g_cfg_vfs.dev_flash.from_string(shared_dev_flash);
  g_cfg_vfs.dev_flash2.from_string(shared_dev_flash2);
  g_cfg_vfs.dev_flash3.from_string(shared_dev_flash3);
  g_cfg_vfs.dev_bdvd.from_string(root + "/dev_bdvd/");
  g_cfg_vfs.games_dir.from_string(root + "/games/");
  g_cfg_vfs.app_home.from_string("");
  auto usb = g_cfg_vfs.dev_usb.get_map();
  for (auto& [name, device] : usb) {
    if (!device.path.empty()) {
      device.path = root + "/dev_usb000/";
    }
  }
  g_cfg_vfs.dev_usb.set_map(std::move(usb));
  g_cfg_vfs.save();
  rpcs3_android.notice("Loaded TeknoParrot VFS config: %s", config_path);
  return true;
}
...
Java_net_rpcs3_RPCS3_initialize(JNIEnv *env, jobject, jstring rootDir) {
  ...
  g_cfg_input.player1.handler.set(pad_handler::keyboard);
...
extern "C" JNIEXPORT jboolean JNICALL Java_net_rpcs3_RPCS3_usbDeviceEvent(
...

  {
    ...
    if (selectedHandler == pad_handler::null) {
      selectedHandler = pad_handler::keyboard;
    }
    ...
    if (selectedHandler != g_cfg_input.player1.handler.get()) {
      ...
      if (selectedHandler == pad_handler::null) {
      ...
      } else if (selectedHandler == pad_handler::keyboard) {
        g_cfg_input.player1.handler.set(pad_handler::keyboard);
        g_cfg_input.player1.device.from_string("Virtual");
      } else {
        ...
        if (selectedHandler != pad_handler::keyboard) {
          std::lock_guard lock(g_virtual_pad_mutex);
  ...
  for (const char c : value) {
    ...
    default:
      if (static_cast<unsigned char>(c) < 0x20) {
        fmt::append(result, "\\u%04x", static_cast<unsigned char>(c));
      } else {
        result += c;
      }
      break;
...
static std::string cfg_to_json(const cfg::_base *entry) {
  if (entry->get_type() == cfg::type::node) {
    const auto *node = static_cast<const cfg::node *>(entry);
    std::string result = "{";
    bool first = true;
    for (const auto *child : node->get_nodes()) {
      if (!first) result += ',';
      first = false;
      fmt::append(result, "%s:%s", json_quote(child->get_name()),
                  cfg_to_json(child));
    }
    result += '}';
    return result;
  ...
  const std::string default_value = entry->def_to_string();
  const auto range = entry->to_list();
  std::string type;

  switch (entry->get_type()) {
  case cfg::type::_bool: type = "bool"; break;
  case cfg::type::_enum: type = "enum"; break;
  case cfg::type::_int:
    type = range.size() == 2 && range[0].find('.') != std::string::npos
               ? "float"
               : "int";
    break;
  case cfg::type::uint:
  case cfg::type::uint128: type = "uint"; break;
  case cfg::type::string: type = "string"; break;
  default: type = "unsupported"; break;
  }

  std::string result = "{";
  fmt::append(result, "\"type\":%s,", json_quote(type));
  if (entry->get_type() == cfg::type::_bool) {
    fmt::append(result, "\"value\":%s,\"default\":%s", value,
                default_value);
  } else {
    fmt::append(result, "\"value\":%s,\"default\":%s", json_quote(value),
                json_quote(default_value));
  }

  if (entry->get_type() == cfg::type::_enum) {
    result += ",\"variants\":[";
    for (usz i = 0; i < range.size(); i++) {
      if (i) result += ',';
      result += json_quote(range[i]);
    }
    result += ']';
  } else if ((type == "int" || type == "uint" || type == "float") &&
             range.size() == 2) {
    fmt::append(result, ",\"min\":%s,\"max\":%s", json_quote(range[0]),
                json_quote(range[1]));
  }
...
static bool unwrap_json_string(std::string_view input, std::string &output) {
...
Java_net_rpcs3_RPCS3_settingsGet(JNIEnv *env, jobject, jstring jpath) {
  ...
  return wrap(env, cfg_to_json(root));
}
...
extern "C" JNIEXPORT jboolean JNICALL Java_net_rpcs3_RPCS3_settingsSet(
  ...
  const std::string json_value = unwrap(env, jvalue);

  auto root = find_cfg_node(&g_cfg, path);

  if (root == nullptr) {
    rpcs3_android.error("settingsSet: node %s not found", path);
    return false;
  }

  std::string value = json_value;
  if (root->get_type() == cfg::type::_enum ||
      root->get_type() == cfg::type::string) {
    if (!unwrap_json_string(json_value, value)) {
      rpcs3_android.error("settingsSet: node %s requires a JSON string", path);
      return false;
    }
  } else if (json_value.empty() || json_value.front() == '"') {
    rpcs3_android.error("settingsSet: node %s passed invalid scalar '%s'", path,
                        json_value);
    return false;
  }
