...
#ifdef ANDROID
template <>
void fmt_class_string<thread_affinity_class>::format(std::string& out, u64 arg)
{
	format_enum(out, arg, [](thread_affinity_class value)
	{
		switch (value)
		{
		case thread_affinity_class::general: return "General";
		case thread_affinity_class::ppu: return "PPU";
		case thread_affinity_class::spu: return "SPU";
		case thread_affinity_class::rsx: return "RSX";
		}
		return unknown;
	});
}
...
{
#ifdef ANDROID
	const thread_affinity_class affinities[] =
	{
	...
	const thread_affinity_class requested = [&]()
	{
		switch (group)
		{
		case thread_class::ppu: return thread_affinity_class::ppu;
		case thread_class::spu: return thread_affinity_class::spu;
		case thread_class::rsx: return thread_affinity_class::rsx;
		default: return thread_affinity_class::general;
		}
	}();

	u64 mask = 0;
	for (usz i = 0; i < std::min<usz>(std::thread::hardware_concurrency(), std::size(affinities)); ++i)
	{
		if (affinities[i] == requested || affinities[i] == thread_affinity_class::general)
		{
			mask |= 1ull << i;
		}
	}

	for (usz i = std::size(affinities); i < std::min<usz>(std::thread::hardware_concurrency(), 64); ++i)
	{
		mask |= 1ull << i;
	}

	return mask ? mask : process_affinity_mask;
#endif
